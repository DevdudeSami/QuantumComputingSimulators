...
Gate QuantumLanguageInterpreter::gateFor(string g) {
  if(g == "X") return X();
  // else if(g == "Y") return Y();
  else if(g == "Z") return Z();
  else if(g == "H") return H();
  // else if(g == "P") return P();
  else if(g == "CNOT") return CNOT();
  else if(g == "TOFF") return TOFF();
  else if(g == "SWAP") return SWAP();
...
string QuantumLanguageInterpreter::execute() {
  ...
  while(getline(file, line)) {
    ...
    if(splitString[0] == "init") {
      comp = new QRegister(stoi(splitString[1]));
      continue;
    }
  ...
  for(ApplicableGate g: gates) {
    comp->applyMultiGate(g.second, g.first);
  }

  if(numberOfMeasurements != 0) {
    cout << "Took " << numberOfMeasurements << " measurements: " << takeMeasurementsInString(*comp, numberOfMeasurements) << endl;
  }
